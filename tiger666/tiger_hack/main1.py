...
def func_apply():
    ...
    (status,output) = commands.getstatusoutput('python ../tigerhack/try.py ./tools/freeq-master/news 50 && cp -r ./tools/freeq-master/news/* ./database/all_news && python ./tools/freeq-master/freeq.py && python ./tools/freeq-master/filter.py && python ./tools/freeq-master/merge.py && ./temp.sh && rm -f ./tools/freeq-master/1.txt && cat ./tools/freeq-master/result.txt | python ./cloud1.py  && ' + 
    ' cp ./tools/freeq-master/result.txt ./database/all_outputtext/' + 'output{}.txt'.format(datetime.now().strftime('%Y%m%d_%H%M%S')) + 
    ' && cp ./temp/output/output.png ./database/all_wordcloud/' + 
    'output{}.png'.format(datetime.now().strftime('%Y%m%d_%H%M%S')) +
    ' && rm -f ./temp/output/* && rm -f ./tools/freeq-master/result.txt && rm -f ./tools/freeq-master/news/* && rm -f ./tools/freeq-master/rawWordsFreEachNews/* && rm -f ./tools/freeq-master/finalWordsFreEachNews/* '
    )
