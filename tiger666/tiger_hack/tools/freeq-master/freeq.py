...
lemmas = {}
with open(os.path.dirname(__file__)+'/lemmas.txt') as fin:
    for line in fin:
        line = line.strip()
...
class Book(object):
    def __init__(self, filepath):
        with open(os.path.dirname(__file__)+'/' + filepath) as bookfile:
...
if __name__ == '__main__':
    ...
    files = os.listdir(os.path.dirname(__file__) + '/news')
    #print len(files)
    for i in range(0,len(files)):
        book = Book('news' + '/' +files[i])

     #   print  os.path.dirname(__file__)+'/news'+'/'+files[i]
        result = book.freq()
        #print result.values()
        #print result
    # Maximum width of the ocurrence column
       ...
       # print "helloworld"
        ...
        output_file = os.path.dirname(__file__)+'/rawWordsFreEachNews' +'/' + files[i]
        if output_file:
